...
CFloatImage greyDecode(char* outdir, char* codefile, int direction, int rad, int maxDiff, int useFilter, char **imList, int numIm)
{
    CByteImage im;
    CShape sh;
    int i, verbose=1;
	...
	if (1) { // save val as pgm
		...
		sprintf(filename, "%s/vresult%d.pgm",outdir, direction);
	...
	if (1) { // save filled val as pgm
		...
		sprintf(filename, "%s/wresult%d.pgm",outdir, direction);
		WriteImageVerb(result, filename, verbose);
	}
